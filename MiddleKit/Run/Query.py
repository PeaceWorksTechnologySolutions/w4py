...
from spark import GenericScanner, GenericParser, GenericASTTraversal, GenericASTTraversalPruningException
import time
...
class Scanner(GenericScanner):
	...
	def t_identifier(self, s):
		r' [a-zA-Z][a-zA-Z0-9]* '
		if self.keywords.has_key(s.lower()):
			keyword = s.lower()
			if keyword == 'none':
				keyword = 'null'
...
class AttrCheck(GenericASTTraversal):
	...
	def n_identifierForKlass(self, node):
		try:
			...
			if self.joinCache.has_key(join):
...
class Query:
	...
	def handleSubklasses(self, tree, contexts, joins, index, fetchReferenced):
		...
		if index >= len(contexts):
			base = contexts[0]
			cols = map( lambda c: "t0.%s" % c, base.klass.allColumns())
			if fetchReferenced:
				fetchKlasses = map(lambda c: c.klass, contexts)
				for context in contexts[1:]:
					cols.extend( map( lambda c: "%s.%s" % (context.alias, c), context.klass.allColumns() ))
			else:
